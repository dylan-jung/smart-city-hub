...
export default function CompanyEditForm({ company, lang }: Props) {
  const router = useRouter();
  const [formData, setFormData] = useState<SolutionCompany>(company);
  ...
  const handleSave = async () => {
    setLoading(true);
    try {
        await updateCompany({ ...formData, companyId: company.companyId }, lang);
  ...
  return (
    <div className="bg-white rounded-lg shadow overflow-hidden p-6 max-w-4xl mx-auto">
        <div className="flex justify-between items-center mb-6 border-b pb-4">
            <h2 className="text-xl font-bold text-gray-900">기업 정보 수정</h2>
        </div>
        ...
        <div className="mt-8 flex justify-end gap-3 pt-6 border-t">
            ...
            <button onClick={handleSave} disabled={loading} className="px-5 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-medium shadow-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
                {loading ? "저장 중..." : "저장"}
            </button>
        </div>
    </div>
